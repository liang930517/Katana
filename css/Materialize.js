...
document.addEventListener("keydown", docHandleKeydown, !0), document.addEventListener("keyup", docHandleKeyup, !0), document.addEventListener("focus", docHandleFocus, !0), document.addEventListener("blur", docHandleBlur, !0), M.initializeJqueryWrapper = function(n, s, o) {
	...
	var e = t || document.body,
		i = {
			...
			Carousel: e.querySelectorAll(".carousel_m:not(.no-autoinit)"),
...
M.anime = function() {
				...
				return _inherits(n, Component), _createClass(n, [{
					...
					value: function() {
						...
						this.$tabLinks.each(function(t) {
							...
							e.addClass("carousel_m-item"), n = n.add(e)
						});
						var t = a('<div class="tabs-content carousel_m carousel_m-slider"></div>');
	...
	function(b) {
		...
		var e = {
			...
			t = function(t) {
				function i(t, e) {
					...
					return (n.el.M_Carousel = n).options = b.extend({}, i.defaults, e), n.hasMultipleSlides = 1 < n.$el.find(".carousel_m-item").length, n.showIndicators = n.options.indicators && n.hasMultipleSlides, n.noWrap = n.options.noWrap || !n.hasMultipleSlides, n.pressed = !1, n.dragged = !1, n.offset = n.target = 0, n.images = [], n.itemWidth = n.$el.find(".carousel_m-item").first().innerWidth(), n.itemHeight = n.$el.find(".carousel_m-item").first().innerHeight(), n.dim = 2 * n.itemWidth + n.options.padding || 1, n._autoScrollBound = n._autoScroll.bind(n), n._trackBound = n._track.bind(n), n.options.fullWidth && (n.options.dist = 0, n._setCarouselHeight(), n.showIndicators && n.$el.find(".carousel_m-fixed-item").addClass("with-indicators")), n.$indicators = b('<ul class="indicators"></ul>'), n.$el.find(".carousel_m-item").each(function(t, e) {
				...
				return _inherits(i, Component), _createClass(i, [{
					...
					value: function(t) {
						...
						if (!this.options.fullWidth) {
							var e = b(t.target).closest(".carousel_m-item").index();
					...
					key: "_handleResize",
					value: function(t) {
						this.options.fullWidth ? (this.itemWidth = this.$el.find(".carousel_m-item").first().innerWidth(), this.imageHeight = this.$el.find(".carousel_m-item.active").height(), this.dim = 2 * this.itemWidth + this.options.padding, this.offset = 2 * this.center * this.itemWidth, this.target = this.offset, this._setCarouselHeight(!0)) : this._scroll()
					}
				}, {
					key: "_setCarouselHeight",
					value: function(t) {
						var i = this,
							e = this.$el.find(".carousel_m-item.active").length ? this.$el.find(".carousel_m-item.active").first() : this.$el.find(".carousel_m-item").first(),
							n = e.find("img").first();
						if (n.length)
							if (n[0].complete) {
								var s = n.height();
					...
					value: function(t) {
						...
						if (!this.noWrap || 0 <= this.center && this.center < this.count) {
							r = this.images[this._wrap(this.center)], b(r).hasClass("active") || (this.$el.find(".carousel_m-item").removeClass("active"), r.classList.add("active"));
						...
						var k = this.$el.find(".carousel_m-item").eq(this._wrap(this.center));
		...
		M.Carousel = t, M.jQueryLoaded && M.initializeJqueryWrapper(t, "carousel_m", "M_Carousel")
	}(cash),
	function(S) {
		"use strict";
		var e = {
